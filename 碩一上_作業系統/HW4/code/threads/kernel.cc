...
Kernel::Kernel(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // Todo ----
        // Spec p.7, -ep: execute using priority scheduling

        else if (strcmp(argv[i], "-ep") == 0) {
            // check if has two args: filename & priority
            ASSERT(i + 2 < argc); 
            char* processName = argv[i + 1]; 
            int priority = atoi(argv[i + 2]);

            // Spec p.6, The priority is between 0 ~ 149
            ASSERT(priority >= 0 && priority <= 149);

            execfile[++execfileNum] = processName;
            execpriority[execfileNum] = priority; 
            cout << "Process added: " << execfile[execfileNum] << " with priority " << priority << "\n";
            i += 2;
        }
        // ---------
...
void Kernel::ExecAll()
{
    for (int i = 1;i <= execfileNum;i++) {
        // Todo ------
        // Parse the priority as an argument.
...
int Kernel::Exec(char* name, int priority)
{
    ...
    // Set the priority for the new thread.
